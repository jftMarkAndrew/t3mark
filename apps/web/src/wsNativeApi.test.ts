...
describe("wsNativeApi", () => {
  ...
  it("forwards git.listOpenPullRequests to the websocket git method", async () => {
    requestMock.mockResolvedValue({ pullRequests: [] });
    const { createWsNativeApi } = await import("./wsNativeApi");

    const api = createWsNativeApi();
    await api.git.listOpenPullRequests({ cwd: "/repo" });

    expect(requestMock).toHaveBeenCalledWith(WS_METHODS.gitListOpenPullRequests, {
      cwd: "/repo",
    });
  });
